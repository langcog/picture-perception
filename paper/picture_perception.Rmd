...
We were curious whether the difference between these Bayesian models and the earlier preregistered frequentist analyses was due to their differing random effect structures. To investigate this question, we ran Bayesian analyses with the same random effects structure as the convergent preregistered models. Thus, we fit parallel exploratory Bayesian mixed effects models for condition both with and without age interactions, in both cases omitting random condition by item slopes. Paralleling the preregistered analyses, there was a significant difference between object and black-and-white line drawing formats, `r bayes_mod_pruned_obj_bw$estimate_print` in the condition model, and the same effect emerged in the model with age interactions `r bayes_mod_age_pruned_obj_bw$estimate_print`. These models provide support for the idea that changes to the random effect structure were responsible for changes in the significance of the black-and-white line drwaing effect between the preregistered and exploratory models.
