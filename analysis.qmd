...
#this code was pasted in from the cogsci manuscript and doesn't work yet
...
Pre-registered analysis - first model
```{r}
...
summary(us_mod)
#failure to converge - check that this is still true with all data
```

Pruned model with age

```{r}
us_mod_age_pruned <- glm(accuracy ~ condition * age_centered,
              data = us_d, family = "binomial")
