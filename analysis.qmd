...
apa_lm <- apa_print(mod_age_pruned)
apa_lm

table <- apa_table(apa_lm$table)
table
...
Visualize error analyses for Kenya 
...
Try a pruned bare model - doesn't converge
...
```{r}
mod_bare_pruned2 <- glmer(accuracy ~ pic_interact * condition + (1 | child),
              data = d, family = "binomial")
summary(mod_bare_pruned2)
```

Keep pruning!

```{r}
mod_bare_pruned3 <- glm(accuracy ~ pic_interact * condition,
              data = d, family = "binomial")
summary(mod_bare_pruned3)
```
...
These are extra individual differences analyses - just playing around now; the null result seems pretty robust
